...
import struct
import tempfile
from aimrocks.merge_operators import UintAddOperator, StringAppendOperator
...
class TestDB(TestHelper):
    def setUp(self):
        ...
        opts = aimrocks.Options(create_if_missing=True)
        self.db = aimrocks.DB(os.path.join(self.db_loc, "test"), opts)
    ...
    def test_options_used_twice(self):
        ...
        with assertRaisesRegex(Exception, expected):
            aimrocks.DB(os.path.join(self.db_loc, "test2"), self.db.options)

    def test_unicode_path(self):
        name = os.path.join(self.db_loc, b'M\xc3\xbcnchen'.decode('utf8'))
        aimrocks.DB(name, aimrocks.Options(create_if_missing=True))
...
class AssocCounter(aimrocks.interfaces.AssociativeMergeOperator):
...
class FullCounter(aimrocks.interfaces.MergeOperator):
    def name(self):
...
class SimpleComparator(aimrocks.interfaces.Comparator):
    def name(self):
        return b'mycompare'
...
class StaticPrefix(aimrocks.interfaces.SliceTransform):
    def name(self):
        return b'static'
...
class TestDBColumnFamilies(TestHelper):
    def setUp(self):
        ...
        self.cf_a = self.db.create_column_family(b'A', aimrocks.ColumnFamilyOptions())
        self.cf_b = self.db.create_column_family(b'B', aimrocks.ColumnFamilyOptions())
