...
class TestFilterPolicy(aimrocks.interfaces.FilterPolicy):
...
class TestMergeOperator(aimrocks.interfaces.MergeOperator):
...
class TestOptions(unittest.TestCase):
    ...
    def test_simple(self):
        ...
        self.assertIsInstance(
            opts.comparator,
            aimrocks.BytewiseComparator)

        self.assertIn(opts.compression,
                      (aimrocks.CompressionType.no_compression,
                       aimrocks.CompressionType.snappy_compression))

        opts.compression = aimrocks.CompressionType.zstd_compression
        self.assertEqual(aimrocks.CompressionType.zstd_compression, opts.compression)

    def test_block_options(self):
        aimrocks.BlockBasedTableFactory(
            block_size=4096,
            filter_policy=TestFilterPolicy(),
            block_cache=aimrocks.LRUCache(100))
    ...
    def test_row_cache(self):
        ...
        opts.row_cache = cache = aimrocks.LRUCache(2 * 1024 * 1024)
